...
public class MethodReferenceTest {
	private static final Logger log = LoggerFactory.getLogger(MethodReferenceTest.class);
	...
	public static void main(String[] args) throws Exception {
		{
			Function<Foo, String> myfunc1 = (Function<Foo, String> & Serializable) Foo::bar;
			Method method = extractMethod(myfunc1);
			log.info("Method 1: {}", method);
		}

		{
			Function<Foo, String> myfunc2 = Foo::bar;
			Method method = extractMethod(myfunc2);
			log.info("Method 2: {}", method);
		}
	}

	public static <T> Method extractMethod(Function<T, ?> methodReference) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		// Get the writeReplace method from the lambda (it actually exists for serialization purposes)
		Serializable asSerializable = (Serializable) methodReference;
		Method writeReplaceMethod = asSerializable.getClass().getDeclaredMethod("writeReplace");
